...
if(in_array($a,['save_quote','update_quote'],true)){
    ...
    try{
        ...
        $laborSummaryMode=count($laborBlocks)===1?$laborBlocks[0][3]:'sin_iva';
        $laborSummaryVat=count($laborBlocks)===1?(float)$laborBlocks[0][4]:21.0;
        ...
        if($a==='update_quote'){
            ...
            $db->prepare('INSERT INTO quotes(project_id,quote_series_key,version_no,quote_category,proposal_name,currency,quote_date,quote_families,quote_template_family,price_list_id,materials_amount,labor_amount,labor_description,subtotal,tax_mode,vat_rate,materials_tax_mode,materials_vat_rate,labor_tax_mode,labor_vat_rate,total,status,notes,responsible_user_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([$pid,$seriesKey,$version,$category,$proposalName!==''?$proposalName:null,'USD',$date,implode(',',$families),$template,$listId,$materials,$labor,$laborDesc,$materials+$labor,'sin_iva',21,$matMode,$matVat,$laborSummaryMode,$laborSummaryVat,$total,$status,$notes,$responsibleUserId]);$qid=(int)$db->lastInsertId();
        }
        ...
        $db->commit();$_SESSION['msg']=$emptyDraft?'Borrador guardado. Podés agregar productos y mano de obra cuando quieras.':($a==='update_quote'?'Presupuesto actualizado.':'Presupuesto creado. Podés seguir editándolo o generar el PDF cuando quieras.');$afterSave=(string)($_POST['after_save']??'edit');$destination=(!$emptyDraft&&$afterSave==='pdf')?'quote_view':'edit_quote';header('Location:index.php?a='.$destination.'&id='.$qid);exit;
