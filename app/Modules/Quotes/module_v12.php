...
if(in_array($a,['save_quote','update_quote'],true)){
    ...
    try{
        ...
        foreach($_POST['items']??[] as $r){
            ...
            $section=trim((string)($r['section_title']??'Materiales'))?:'Materiales';$order=(int)($r['block_order']??0);$manual=!empty($r['is_manual']);
            if($manual){
                ...
                $items[]=[null,1,'Electricidad','Otro',$section,$order,$sku,$desc,$u,$qty,$unit,null,$sub,$sort++];
                continue;
            }
        ...
        $date=$_POST['quote_date']??date('Y-m-d');$status=$_POST['status']??'borrador';$category=$_POST['quote_category']??'general';$notes=trim((string)($_POST['notes']??''));$laborDesc=count($laborBlocks)===1?$laborBlocks[0][1]:(count($laborBlocks)>1?'Mano de obra por sectores':'');
        $db->beginTransaction();
        if($a==='update_quote'){
...
(function(){
 ...
 const originalCalc=window.calcTotals;window.calcTotals=function(){let mat=0;document.querySelectorAll('.material-block tr').forEach(tr=>{const manual=tr.dataset.manual==='1',id=tr.dataset.product,qty=Number(tr.querySelector('.qty')?.value||0),price=Number(tr.querySelector('.unit-price')?.value||0),valid=manual||!!id,sub=valid?qty*price:0;const c=tr.querySelector('.sub');if(c)c.textContent=valid?money(sub):'—';mat+=sub});let labor=0,laborTaxed=0;document.querySelectorAll('.labor-block').forEach(b=>{const a=Number(b.querySelector('.labor-amount').value||0),m=b.querySelector('.labor-tax').value,v=Number(b.querySelector('.labor-vat').value||21);labor+=a;laborTaxed+=m==='mas_iva'?a*(1+v/100):a});const mm=document.getElementById('materialsTax').value,mv=Number(document.getElementById('materialsVat').value||21),matTaxed=mm==='mas_iva'?mat*(1+mv/100):mat;document.getElementById('materialsTotal').textContent=money(matTaxed);document.getElementById('laborTotal').textContent=money(laborTaxed);document.getElementById('grandTotal').textContent=money(matTaxed+laborTaxed)};
 calcTotals();
})();
</script>
HTML;
