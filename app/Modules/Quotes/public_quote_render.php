...
try{
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'&&($_POST['public_action']??'')==='accept'){
  try{
   ...
   $db->beginTransaction();$u=$db->prepare("UPDATE quotes SET status='aprobado_inicial',followup_closed_at=?,next_followup_date=NULL WHERE id=? AND status IN ('borrador','enviado')");$u->execute([$acceptedAt,$id]);if($u->rowCount()!==1)throw new Exception('El presupuesto ya fue aceptado o dejó de estar vigente.');
