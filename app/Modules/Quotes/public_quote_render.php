...
try{
 ...
 $s=$db->prepare('SELECT q.*,p.project_number,p.name project_name,p.engineering_pct,c.business_name,c.contact_name,c.cuit FROM quotes q JOIN projects p ON p.id=q.project_id JOIN clients c ON c.id=p.client_id WHERE q.id=?');$s->execute([$id]);$quote=$s->fetch()?:[];
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'&&($_POST['public_action']??'')==='accept'){
  try{
   ...
   if(!$quote||in_array($quote['status'],['aprobado_inicial','aprobado_definitivo','final','rechazado'],true))throw new Exception('Esta versión ya no está disponible para aceptación.');
   ...
   $db->beginTransaction();$u=$db->prepare("UPDATE quotes SET status='aprobado_inicial',locked_at=COALESCE(locked_at,NOW()),followup_closed_at=?,next_followup_date=NULL WHERE id=? AND status IN ('borrador','enviado')");$u->execute([$acceptedAt,$id]);if($u->rowCount()!==1)throw new Exception('El presupuesto ya fue aceptado o dejó de estar vigente.');
   $engineeringPct=(float)($quote['engineering_pct']??0);$engineeringAmount=round((float)($quote['total']??0)*$engineeringPct/100,2);$currency=(string)($quote['currency']??'USD');
   if($engineeringAmount>0){$charge=$db->prepare("INSERT INTO charges(project_id,quote_id,charge_date,type,currency,description,amount) VALUES(?,?,?,'ingenieria',?,?,?)");$charge->execute([(int)$quote['project_id'],$id,substr($acceptedAt,0,10),$currency,'Adelanto de ingeniería '.$engineeringPct.'% · '.($quote['proposal_name']?:'Presupuesto').' · v'.$quote['version_no'],$engineeringAmount]);}
   $db->prepare("UPDATE projects SET status='aprobado' WHERE id=?")->execute([(int)$quote['project_id']]);
...
(function(){
 ...
 async function appendQuote(out){const s=document.getElementById('quoteSheet');if(!s)return;if(typeof waitForImages==='function')await waitForImages(s);const c=await html2canvas(s,{scale:2,useCORS:true,backgroundColor:'#fff',logging:false,width:s.scrollWidth,height:s.scrollHeight,windowWidth:s.scrollWidth,windowHeight:s.scrollHeight}),pw=595.28,ph=841.89,cssPage=s.scrollWidth*ph/pw,top=s.getBoundingClientRect().top,breaks=[0],rows=[...s.querySelectorAll('.equipment tr')].map(r=>r.getBoundingClientRect().bottom-top).filter(v=>v>0&&v<s.scrollHeight);let start=0;while(start+cssPage<s.scrollHeight){const limit=start+cssPage-8,candidates=rows.filter(v=>v>start+80&&v<=limit);let next=candidates.length?Math.max(...candidates):limit;if(next<=start+20)next=Math.min(start+cssPage,s.scrollHeight);breaks.push(next);start=next}breaks.push(s.scrollHeight);const ratio=c.height/s.scrollHeight;for(let i=0;i<breaks.length-1;i++){const sy=Math.round(breaks[i]*ratio),ey=Math.round(breaks[i+1]*ratio),slice=document.createElement('canvas');slice.width=c.width;slice.height=Math.max(1,ey-sy);slice.getContext('2d').drawImage(c,0,sy,c.width,slice.height,0,0,c.width,slice.height);const png=await out.embedPng(slice.toDataURL('image/png')),h=png.height*(pw/png.width),page=out.addPage([pw,ph]);page.drawImage(png,{x:0,y:ph-h,width:pw,height:h})}}
