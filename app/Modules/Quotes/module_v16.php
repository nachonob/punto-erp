...
(function(){
 ...
 function bases(){let mat=0;document.querySelectorAll('.material-block tr').forEach(tr=>{const manual=tr.dataset.manual==='1'||tr.querySelector('input[name$="[is_manual]"]')?.value==='1',id=tr.dataset.product||tr.querySelector('.product-id')?.value,qty=Number(tr.querySelector('.qty')?.value||0),price=Number(tr.querySelector('.unit-price')?.value||0),discount=Math.max(0,Math.min(100,Number(tr.querySelector('.item-discount')?.value||0)));if(manual||id)mat+=qty*price*(1-discount/100)});let labor=0;document.querySelectorAll('.labor-block').forEach(b=>labor+=Number(b.querySelector('.labor-amount')?.value||0));return{mat,labor,total:mat+labor}}
 window.calcDiscounts=function(){const b=bases(),matMode=document.getElementById('materialsTax')?.value||'sin_iva',matVat=Number(document.getElementById('materialsVat')?.value||0),matTotal=matMode==='mas_iva'?b.mat*(1+matVat/100):b.mat;let laborTotal=0;document.querySelectorAll('.labor-block').forEach(block=>{const amount=Number(block.querySelector('.labor-amount')?.value||0),mode=block.querySelector('.labor-tax')?.value||'sin_iva',vat=Number(block.querySelector('.labor-vat')?.value||0);laborTotal+=mode==='mas_iva'?amount*(1+vat/100):amount});const budgetTotal=matTotal+laborTotal;let total=0;rows.querySelectorAll('.discount-row').forEach(r=>{const scope=r.querySelector('.discount-scope').value,type=r.querySelector('.discount-type').value,val=Math.max(0,Number(r.querySelector('.discount-value').value||0));let base=scope==='materials'?b.mat:scope==='labor'?b.labor:budgetTotal;if(scope==='item'){const s=r.querySelector('.discount-item'),o=s.selectedOptions[0];if(o?.dataset.type==='material'){const tr=[...document.querySelectorAll('.material-block tr')].find(x=>String(x.querySelector('.product-id')?.value)===s.value);base=tr?Number(tr.querySelector('.qty')?.value||0)*Number(tr.querySelector('.unit-price')?.value||0)*(1-Math.max(0,Math.min(100,Number(tr.querySelector('.item-discount')?.value||0)))/100):0}else if(o?.dataset.type==='labor'){const n=Number(s.value.replace('labor-',''));const lb=document.querySelectorAll('.labor-block')[n];base=lb?Number(lb.querySelector('.labor-amount')?.value||0):0}else base=0}total+=type==='percentage'?base*Math.min(100,val)/100:Math.min(base,val)});card.querySelector('#discountTotal').textContent=money(total);const gt=document.getElementById('grandTotal');if(gt)gt.textContent=money(Math.max(0,budgetTotal-total))}
