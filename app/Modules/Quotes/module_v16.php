...
(function(){
 ...
 function bases(){let mat=0;document.querySelectorAll('.material-block tr').forEach(tr=>{const id=tr.dataset.product||tr.querySelector('.product-id')?.value,qty=Number(tr.querySelector('.qty')?.value||0),price=Number(tr.querySelector('.unit-price')?.value||0),discount=Math.max(0,Math.min(100,Number(tr.querySelector('.item-discount')?.value||0)));if(id)mat+=qty*price*(1-discount/100)});let labor=0;document.querySelectorAll('.labor-block').forEach(b=>labor+=Number(b.querySelector('.labor-amount')?.value||0));return{mat,labor,total:mat+labor}}
