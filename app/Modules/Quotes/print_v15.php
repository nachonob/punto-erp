...
try{
    ...
    $stmt=$imageDb->prepare("SELECT DISTINCT p.id,p.sku,p.image_data,p.image_mime FROM quote_items qi JOIN products p ON p.id=qi.product_id WHERE qi.quote_id=? AND qi.product_id IS NOT NULL");
    $stmt->execute([$id]);
...
<script>
(function(){
 const images=$imagesJson;if(!images||!Object.keys(images).length)return;
 document.querySelectorAll('.equipment tbody tr').forEach(row=>{
  ...
  const sku=(cells[1]?.textContent||'').trim(),src=images[sku];if(!src)return;
  const cell=cells[0];if(cell.querySelector('img'))return;
