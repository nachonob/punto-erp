...
<?php if($actions):?><p class="muted">Elegí el próximo estado comercial del presupuesto.</p><div class="actions"><?php foreach($actions as $value=>$label):$confirmation=$value==='aprobado_inicial'?'¿Desea aprobar este presupuesto como Aceptado inicial?':($value==='aprobado_definitivo'?'¿Desea aprobar este presupuesto como Aceptado definitivo?':'¿Desea marcar este presupuesto como rechazado?');?><form method="post" action="?a=quote_status" onsubmit="return confirm('<?=qse($confirmation)?>')"><input type="hidden" name="csrf" value="<?=qse($_SESSION['csrf'])?>"><input type="hidden" name="quote_id" value="<?=$id?>"><input type="hidden" name="status" value="<?=qse($value)?>"><button class="btn <?=$value==='rechazado'?'danger':''?>" type="submit"><?=qse($label)?></button></form><?php endforeach;?></div>
