...
try{
 ...
 $s=$db->prepare('SELECT * FROM quotes WHERE id=?');$s->execute([$id]);$qr=$s->fetch()?:[];$breakdown=quoteFinancialBreakdown($db,$qr);$baseTotal=(float)$breakdown['total'];$mat=(float)$breakdown['materials_input'];$lab=(float)$breakdown['labor_input'];
